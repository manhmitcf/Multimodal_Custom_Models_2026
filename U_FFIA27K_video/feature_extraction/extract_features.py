...
def entry_to_base_row(entry: List[Any]) -> Dict[str, Any]:
    ...
    return {
        "sample_key": sample_key_from_path(video_path),
...
class VideoFeatureDataset(Dataset):
    ...
    def _decode_row(self, index: int) -> Tuple[int, np.ndarray]:
        row = entry_to_base_row(self.entries[index])
        try:
            sample = _decode_center_image(row["input_path"], row["label_id"], self.image_size)
        except Exception as exc:
            logger.warning(f"Decord failed for '{row['input_path']}', falling back to OpenCV. Error: {exc}")
            sample = _decode_center_image_cv2(row["input_path"], row["label_id"], self.image_size)
        return index, sample["image_form"]

    def _preload_images_to_ram(self) -> None:
        logger.info(f"Feature extraction stage: preloading video frames into RAM (samples={len(self.entries)})")
        cache: List[np.ndarray | None] = [None] * len(self.entries)
        total_bytes = 0
        with concurrent.futures.ThreadPoolExecutor(max_workers=self.num_workers) as executor:
            futures = {executor.submit(self._decode_row, index): index for index in range(len(self.entries))}
            for future in tqdm(
        ...
        logger.info(
            f"Feature extraction stage: video RAM preload completed (samples={len(self.image_cache)}, cache_size_mb={cache_size_mb:.1f})"
        )
    ...
    def __getitem__(self, index: int) -> Dict[str, Any]:
        ...
        if self.image_cache is not None:
            ...
            _, image_form = self._decode_row(index)
...
def default_checkpoint_path(config: TrainConfig, model: VideoModel, fold_index: int | None = None) -> Path:
    ...
    if fold_index is not None:
        return model_dir / f"fold_{fold_index:02d}" / "video_best.pt"
    return model_dir / "video_best.pt"
...
def resolve_checkpoint_path(
    ...
    if fold_index is None:
        ...
        checkpoint_path = (
            Path(cv_dir_value) / f"fold_{fold_index:02d}" / "video_best.pt"
            if cv_dir_value
