...
def test_parameter_budget():
    print("\n" + "=" * 65)
    print("TEST 1: AUDIO TIE-BREAKERS TOURNAMENT PARAMETER BUDGET (< 5.0M)")
    print("=" * 65)
    ...
    print(f"  - Pairwise Audio Tournament Fusion:   {f_params:,}")
...
def test_tournament_forward_and_pairwise():
    print("\n" + "=" * 65)
    print("TEST 2: TOURNAMENT 2-LEVEL FORWARD PASS & 3 AUDIO TIE-BREAKERS")
    print("=" * 65)
    ...
    # 2. Check Level 2 Pairwise Boundaries B12, B23, B13 and Audio Referees
    ...
    # Check that audio referee logits and gammas are present
    assert "logit_12_a" in out and "gamma_12" in out
    assert "logit_23_a" in out and "gamma_23" in out
    assert "logit_13_a" in out and "gamma_13" in out
    ...
    print("[PASSED] Tournament 2-level forward pass with 3 Audio Tie-Breakers verified!")
...
def test_all_8_audio_tie_breaker_combinations():
    print("\n" + "=" * 65)
    print("TEST 5: ALL 8 AUDIO TIE-BREAKER COMBINATIONS (ABLATION MATRIX)")
    print("=" * 65)
    ...
    combinations = [
        ...
        (True,  False, False, "Audio Tie-Breaker B12 Only"),
        (False, True,  False, "Audio Tie-Breaker B23 Only"),
        (False, False, True,  "Audio Tie-Breaker B13 Only"),
        (True,  True,  False, "Audio Tie-Breakers B12 + B23"),
        (True,  False, True,  "Audio Tie-Breakers B12 + B13"),
        (False, True,  True,  "Audio Tie-Breakers B23 + B13"),
        (True,  True,  True,  "All 3 Audio Tie-Breakers (B12 + B23 + B13)"),
    ]

    from config.train_config import AudioTieBreakersConfig

    for idx, (b12, b23, b13, desc) in enumerate(combinations, 1):
        # Validate Pydantic AudioTieBreakersConfig
        tb_pydantic = AudioTieBreakersConfig(enable_b12=b12, enable_b23=b23, enable_b13=b13)
        model = MultimodalSOTANet(num_frames=2, tie_breakers=tb_pydantic)
        th = model.fusion.tournament_head

        # Check sub-modules existence
        if b12:
            assert th.head_b12_a is not None and th.gamma_12 is not None
        else:
            assert th.head_b12_a is None and th.gamma_12 is None

        if b23:
            assert th.head_b23_a is not None and th.gamma_23 is not None
        else:
            assert th.head_b23_a is None and th.gamma_23 is None

        if b13:
            assert th.head_b13_a is not None and th.gamma_13 is not None
        else:
            assert th.head_b13_a is None and th.gamma_13 is None
    ...
    seed_everything(42)
    m_b12 = MultimodalSOTANet(num_frames=2, tie_breakers=AudioTieBreakersConfig(enable_b12=True, enable_b23=False, enable_b13=False)).eval()
    seed_everything(42)
    m_b23 = MultimodalSOTANet(num_frames=2, tie_breakers=AudioTieBreakersConfig(enable_b12=False, enable_b23=True, enable_b13=False)).eval()
    with torch.no_grad():
    ...
    print("[PASSED] All 8 Audio Tie-Breaker ablation combinations verified successfully!")
...
if __name__ == "__main__":
    print("\n" + "=" * 65)
    print("RUNNING MANDATORY AUDIO TIE-BREAKERS ARCHITECTURE VERIFICATION TEST SUITE")
    print("=" * 65)
    ...
    test_all_8_audio_tie_breaker_combinations()
    test_consistent_video_transform()
    test_convnext_layerscale_and_droppath()

    print("\n" + "=" * 65)
    print("ALL AUDIO TIE-BREAKERS TESTS PASSED SUCCESSFULLY! (100% READY)")
    print("=" * 65 + "\n")
