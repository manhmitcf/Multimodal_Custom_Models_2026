...
def mode_dry_run(args: argparse.Namespace) -> None:
    ...
    if loss_type == "pairwise_tournament" or "p_w_over_m" in out:
        ...
        loss_fn = PairwiseTournamentLoss(
            ...
            teacher_feat_v_dim=getattr(config, "teacher_feat_v_dim", 768),
            teacher_feat_a_dim=getattr(config, "teacher_feat_a_dim", 512),
        ).to(device)
    else:
    ...
    if getattr(config, "enable_kd", True):
        v_dim = getattr(config, "teacher_feat_v_dim", 768)
        a_dim = getattr(config, "teacher_feat_a_dim", 512)
        ...
        target_dict["teacher_feat_video"] = torch.randn(2, v_dim, device=device)
        target_dict["teacher_feat_audio"] = torch.randn(2, a_dim, device=device)
        target_dict["teacher_feat_map_video"] = torch.randn(2, v_dim, 7, 7, device=device)
    ...
    if getattr(config, "enable_kd", True):
        ...
        if os.path.exists(v_ckpt) and os.path.exists(a_ckpt):
            ...
            try:
                ...
                t_out = teachers(dummy_video, dummy_audio)
                if isinstance(t_out, dict):
                    t_v = t_out["teacher_logits_video"]
                    t_a = t_out["teacher_logits_audio"]
                    t_fv = t_out["teacher_feat_video"]
                    t_fa = t_out["teacher_feat_audio"]
                    t_fmv = t_out["teacher_feat_map_video"]
                    print(f"    - Teachers operational: Video Logits {tuple(t_v.shape)}, Audio Logits {tuple(t_a.shape)}, Feat V {tuple(t_fv.shape)}, Feat A {tuple(t_fa.shape)}, FeatMap V {tuple(t_fmv.shape)} on {device}")
                else:
                    t_v, t_a = t_out
