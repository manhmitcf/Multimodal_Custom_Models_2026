...
from models import LiteFFIANet, DualStreamFishNet
...
def extract_multimodal_features(
    ...
    """
    Extracts all feature groups (Spatial, Motion, Audio Frequency & Rhythm, Fused representation,
    Sync Score, Modality Weights) and persists them as .npy arrays.
    """
            ...
            for key, val in out.items():
                if isinstance(val, torch.Tensor):
                    if key not in features:
                        features[key] = []
                    features[key].append(val.detach().cpu().numpy())
...
if __name__ == "__main__":
    import argparse
    parser = argparse.ArgumentParser(description="Multimodal Feature Extractor")
    ...
    frontend = AudioFrontend(train_cfg.audio_features)
    if train_cfg.model.backbone in ("DualStreamFishNet", "dual_stream_fish_net"):
        model = DualStreamFishNet(
