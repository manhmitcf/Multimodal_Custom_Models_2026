...
class VideoFeaturesConfig(BaseModel):
    ...
    frame_policy: str = Field(default="uniform", description="Frame extraction policy: 'uniform', 'quarter', 'center', 'three_quarters', 'end', or 'random'.")
...
class AudioFeaturesConfig(BaseModel):
    ...
    mel_bins: int = Field(default=128, description="Number of output frequency bins (128).")
    fmin: int = Field(default=50, description="Minimum frequency in Hz.")
    fmax: int = Field(default=64000, description="Maximum frequency in Hz (Nyquist limit at 128kHz).")
    frontend_type: str = Field(default="stft_gem", description="Audio frontend representation: 'stft_gem' (Linear STFT with GeM p=3 peak-preserving pooling) or 'mel'.")
    gem_p: float = Field(default=3.0, gt=0.0, description="Generalized Mean (GeM) power exponent for spectral peak preservation.")
