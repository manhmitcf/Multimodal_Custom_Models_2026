...
class AudioFeaturesConfig(BaseModel):
    ...
    use_tkeo: bool = Field(default=False, description="Enable Teager-Kaiser Energy Operator Adaptive Pre-Emphasis.")
...
class TrainConfig(BaseModel):
    ...
    teacher_video_model: str = Field(default="ConvNeXtTiny", description="Video teacher model architecture.")
    teacher_video_ckpt: str = Field(default="teachers/ConvNeXtTiny/DL_video/checkpoint/convnext_tiny/video_best.pt", description="Path to video teacher checkpoint.")
    ...
    teacher_feat_v_dim: int = Field(default=768, description="Feature embedding dim of Video Teacher (ConvNeXt-Tiny).")
    teacher_feat_a_dim: int = Field(default=512, description="Feature embedding dim of Audio Teacher (PANNS_Cnn6).")
    kd_temperature_video: float = Field(default=3.0, ge=0.1, description="Softmax temperature for video KD.")
    kd_temperature_audio: float = Field(default=8.0, ge=0.1, description="Softmax temperature for audio KD.")
    kd_alpha_video: float = Field(default=0.50, ge=0.0, le=1.0, description="KD loss weight alpha for video (50% CE / 50% KD).")
    kd_alpha_audio: float = Field(default=0.50, ge=0.0, le=1.0, description="KD loss weight alpha for audio (50% CE / 50% KD).")
