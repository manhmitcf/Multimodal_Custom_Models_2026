...
class TrainConfig(BaseModel):
    ...
    lr_scheduler: str = Field(default="onecycle", description="Learning rate scheduler: 'onecycle' (OneCycleLR step-level policy).")
    pct_start: float = Field(default=0.05, ge=0.0, le=1.0, description="Percentage of cycle spent increasing LR (warmup).")
    div_factor: float = Field(default=25.0, gt=0, description="Determines initial learning rate: initial_lr = max_lr / div_factor.")
    final_div_factor: float = Field(default=1000.0, gt=0, description="Determines minimum learning rate: min_lr = initial_lr / final_div_factor.")
