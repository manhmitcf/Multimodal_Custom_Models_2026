...
import torch
from features.motion_kinematics import FishMotionKinematics9Ch
from models.multimodal_sota_net import MultimodalBoundaryAwareNet


def test_9ch_kinematics_extractor():
    print('=' * 65)
    print('TEST 1: 9-CHANNEL MOTION KINEMATICS EXTRACTOR (T=2 FRAMES, NO DIVERGENCE)')
    print('=' * 65)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    kinematics = FishMotionKinematics9Ch(image_size=224).to(device)

    B, T = 4, 2
    dummy_rgb = torch.rand(B, T, 3, 224, 224, device=device, requires_grad=True)

    frames_9ch, summary = kinematics(dummy_rgb)

    print(f'Output frames_9ch shape: {list(frames_9ch.shape)}')
    print(f'Output summary shape:    {list(summary.shape)}')

    assert frames_9ch.shape == (B, T, 9, 224, 224), f'Shape mismatch: {frames_9ch.shape}'
    ...
    ch_names = [
        ...
        'Ch 7: Frame Diff dI',
        'Ch 8: Convective Acc |a|'
    ]

    for c in range(9):
        ch_slice = frames_9ch[:, :, c]
...
def test_full_model_9ch():
    print("\n" + "=" * 65)
    print('TEST 2: FULL MULTIMODAL MODEL WITH 9-CH CONVNEXT-NANO & TOURNAMENT')
    print('=' * 65)

    device = torch.device('cuda' if torch.cuda.is_available() else 'cpu')
    model = MultimodalBoundaryAwareNet(in_chans=9, num_frames=2).to(device)
    model.eval()
    ...
    print(f'  - ConvNeXt-Nano 9-ch Video: {v_params:,} ({v_params/1e6:.3f} M)')
