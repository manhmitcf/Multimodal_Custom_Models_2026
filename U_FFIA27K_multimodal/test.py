...
def test_parameter_budget(
    ...
    if verbose:
        ...
        print(f"  - Audio Backbone (Harmonic 1D-CRNN 256k)    : {stats['audio_backbone']:,} ({stats['audio_backbone']/1e6:.3f} M)")
...
def test_kinematics_and_frontend(model: nn.Module, config: TrainConfig, device: torch.device, verbose: bool = True) -> bool:
    ...
    if hasattr(model, "audio_frontend") and model.audio_frontend is not None:
        ...
        if expected_bins not in stft_feat_train.shape:
            raise ValueError(f"STFT frequency bins mismatch! Expected {expected_bins} in shape, got {stft_feat_train.shape}")
        ...
        if expected_bins not in stft_feat_eval.shape:
            raise ValueError(f"STFT frequency bins mismatch in eval! Expected {expected_bins} in shape, got {stft_feat_eval.shape}")

        if verbose:
            aug_str = (
                f"2D SpecAugment: FreqMask={getattr(config.audio_features, 'freq_mask_max', 32)}bins, "
