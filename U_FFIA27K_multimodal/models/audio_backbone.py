...
class FrequencyAttentionBlock(nn.Module):
    """
    Frequency Attention Mechanism (Group 3a Feature).
    Applies learnable channel-like attention across the 128 Mel frequency bins.
    Suppresses <500Hz pump/aerator noise while amplifying 2-8kHz feeding splash band.
    """
    def __init__(self, n_mels: int = 128, reduction: int = 4) -> None:
        super().__init__()
