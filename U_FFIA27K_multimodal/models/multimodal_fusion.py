...
def _parse_tie_breakers(
    ...
    dictionaries, or objects with attributes for dual tie-breakers ablation.
    """
    if tie_breakers is not None:
        ...
        def _extract(v, default):
            if isinstance(v, dict):
                ...
                if "enable_video" in v or "enable_audio" in v:
                    return bool(v.get("enable_video", False) or v.get("enable_audio", False))
                if "video" in v or "audio" in v:
                    return bool(v.get("video", False) or v.get("audio", False))
                return default
            if hasattr(v, "enable"):
                return bool(getattr(v, "enable"))
            if hasattr(v, "enable_video") or hasattr(v, "enable_audio"):
                return bool(getattr(v, "enable_video", False) or getattr(v, "enable_audio", False))
            return bool(v)
